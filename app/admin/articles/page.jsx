...
// //   FiX, FiHash, FiImage, FiFileText, FiAlignLeft, FiUpload, FiTrash, FiAlertCircle,
// // } from "react-icons/fi";
// // import { articlesAdminAPI, categoriesAdminAPI, authorsAdminAPI } from "@/services/adminAPI";
// // import ConfirmDialog from "@/components/admin/ConfirmDialog";
// // import Link from "next/link";

// // // ── Inline image-error notification shown below the Content section ───────────
...
// //       <h3 className="text-[#F5C645] font-semibold flex items-center gap-2 text-sm uppercase tracking-wider">
// //         {icon}{title}
// //       </h3>
// //       {children}
// //     </div>
...
// //         <span className={`text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${tagColor[block.type]}`}>
// //           {block.type}
// //         </span>
// //         <div className="flex items-center gap-1">
// //           <button type="button" onClick={() => onMove(index, -1)} disabled={index === 0}
// //             className="w-6 h-6 flex items-center justify-center text-gray-500 hover:text-white disabled:opacity-20 cursor-pointer text-xs" title="Move up">▲</button>
// //           <button type="button" onClick={() => onMove(index, 1)} disabled={index === total - 1}
// //             className="w-6 h-6 flex items-center justify-center text-gray-500 hover:text-white disabled:opacity-20 cursor-pointer text-xs" title="Move down">▼</button>
// //           <button type="button" onClick={() => onRemove(index)}
// //             className="w-6 h-6 flex items-center justify-center text-red-400 hover:text-red-300 cursor-pointer ml-1" title="Remove">
// //             <FiTrash size={13} />
// //           </button>
// //         </div>
// //       </div>
...
// //             <p className="text-gray-500 text-xs">Only .webp · Under 100 KB</p>
...
// //                     {block.imagePreview ? 'Replace image' : 'Upload .webp image'}
// //                   </span>
// //                 </div>
...
// //                   <img src={block.imagePreview} alt="preview" className="w-20 h-20 rounded-lg object-cover" />
...
// //             <input type="text" value={block.alt} onChange={(e) => onUpdate(index, 'alt', e.target.value)}
// //               placeholder="Alt text (SEO)" className={inp} />
// //             <input type="text" value={block.caption} onChange={(e) => onUpdate(index, 'caption', e.target.value)}
// //               placeholder="Caption (optional)" className={inp} />
// //           </div>
// //         )}
// //       </div>
// //     </div>
...
// //   const [imageToast, setImageToast] = useState('');   // image error message
// //   const [saving, setSaving]         = useState(false);
// //   const [confirm, setConfirm]       = useState({ open: false, id: null, title: '' });
// //   const [contentErrors, setContentErrors] = useState({}); // Track per-block errors
// //   const [selectedCategoryName, setSelectedCategoryName] = useState(''); // Track selected category name for author mapping
...
// //   // Helper function to get author based on selected category
...
// //     // Store category name for display purposes
// //     setSelectedCategoryName(selectedCategory.name);
    
// //     // Find author whose category matches the selected category name
// //     // Handle different data types for author.category (string, object with name, etc.)
// //     const matchedAuthor = authors.find(author => {
// //       if (!author.category) return false;
      
// //       // Get the category value as string
// //       let authorCategory = '';
// //       if (typeof author.category === 'string') {
// //         authorCategory = author.category;
// //       } else if (author.category && typeof author.category === 'object') {
// //         // If category is an object, try to get name or _id
// //         authorCategory = author.category.name || author.category._id || '';
// //       } else {
// //         authorCategory = String(author.category);
// //       }
      
// //       // Compare with selected category name (case insensitive)
...
// //   // Handle category change and auto-set author
// //   function handleCategoryChange(categoryId) {
// //     setFormData(prev => ({ ...prev, category: categoryId }));
// //     setFormErrors(prev => ({ ...prev, category: '' }));
    
// //     // Auto-set author based on selected category
// //     const autoAuthorId = getAuthorByCategory(categoryId);
// //     if (autoAuthorId) {
// //       setFormData(prev => ({ ...prev, author: autoAuthorId }));
// //     } else {
...
// //     if (!formData.imageAlt.trim()) e.imageAlt = 'ImageAlt is required.';
// //     if (!formData.keywords.trim()) e.keywords = 'Keywords are required.';
// //     if (!formData.tags.trim()) e.tags = 'Tags are required.';
    
// //     // Featured Image validation: either existing image (editing) or new file uploaded
// //     if (!editingArticle && !mainImgFile) {
// //       e.mainImage = 'Featured image is required.';
// //     }
    
// //     // Author validation: required based on category
// //     if (!formData.author && formData.category) {
// //       const selectedCategory = categories.find(cat => cat._id === formData.category);
// //       if (selectedCategory) {
// //         e.author = `Author is required for ${selectedCategory.name} category. Please ensure an author exists for this category.`;
// //       }
// //     }
    
// //     // Content validation: at least one paragraph block with non-empty text
// //     const hasValidParagraph = contentBlocks.some(block => 
// //       block.type === 'paragraph' && block.text && block.text.trim().length > 0
// //     );
// //     if (!hasValidParagraph) {
// //       e.content = 'At least one paragraph block with text is required.';
// //     }
...
// //     if (type === 'image') {
// //       b.src = ''; b.imageFile = null; b.imagePreview = ''; b.alt = ''; b.caption = '';
// //     }
// //     setContentBlocks((p) => [...p, b]);
// //     // Clear content error when adding a block
...
// //     // Clear error for this specific block if it exists
// //     if (contentErrors[i] && contentErrors[i][key]) {
// //       setContentErrors(prev => ({ ...prev, [i]: { ...prev[i], [key]: '' } }));
// //     }
// //     // Clear general content error when updating any block
// //     if (formErrors.content) setFormErrors(prev => ({ ...prev, content: '' }));
// //   }
...
// //       setContentBlocks((p) =>
// //         p.map((b, idx) =>
// //           idx === i ? { ...b, imageFile: file, imagePreview: r.result, src: '__NEW_UPLOAD__' } : b
// //         )
// //       );
// //       // Clear image error for this block if any
// //       if (contentErrors[i] && contentErrors[i].src) {
// //         setContentErrors(prev => ({ ...prev, [i]: { ...prev[i], src: '' } }));
// //       }
// //     };
...
// //     setContentBlocks((p) =>
// //       p.map((b, idx) => idx === i ? { ...b, imageFile: null, imagePreview: '', src: '' } : b)
// //     );
// //   }

// //   function removeBlock(i) {
// //     setContentBlocks((p) => p.filter((_, idx) => idx !== i));
// //     // Remove errors for this block
...
// //         const newKey = parseInt(key) > i ? parseInt(key) - 1 : parseInt(key);
// //         reindexed[newKey] = newErrors[key];
// //       });
// //       return reindexed;
// //     });
// //     // Clear content error if needed
// //     if (formErrors.content) setFormErrors(prev => ({ ...prev, content: '' }));
// //   }
...
// //     // Re-index errors after move
// //     const reindexed = {};
// //     Object.keys(contentErrors).forEach(key => {
// //       const idx = parseInt(key);
// //       if (idx === i) reindexed[to] = contentErrors[i];
// //       else if (idx === to) reindexed[i] = contentErrors[to];
// //       else reindexed[idx] = contentErrors[idx];
// //     });
// //     setContentErrors(reindexed);
// //   }

// //   // Validate content blocks before submission
...
// //           blockErr.text = 'Paragraph text is required.';
// //         } else {
...
// //         if (!block.text || block.text.trim().length === 0) {
// //           blockErr.text = 'Subheading text is required.';
// //         } else {
// //           // Subheading alone doesn't count for paragraph requirement, but we still validate
// //         }
// //       }
// //       if (block.type === 'pullquote') {
// //         if (!block.text || block.text.trim().length === 0) {
// //           blockErr.text = 'Quote text is required.';
// //         }
// //       }
// //       if (block.type === 'image') {
// //         if (!block.src && !block.imageFile) {
// //           blockErr.src = 'Image is required for image block.';
// //         }
// //       }
// //       if (Object.keys(blockErr).length > 0) {
// //         errors[idx] = blockErr;
// //       }
// //     });
...
// //     // First validate content blocks
// //     const { hasValidParagraph, blockErrors } = validateContentBlocks();
    
// //     // Then run general validation
// //     const generalErrors = {};
// //     if (!formData.title.trim())    generalErrors.title    = 'Title is required.';
// //     if (!formData.slug.trim())     generalErrors.slug     = 'Slug is required.';
// //     if (!formData.category)        generalErrors.category = 'Category is required.';
// //     if (!formData.excerpt.trim())  generalErrors.excerpt  = 'Excerpt is required.';
// //     if (!formData.readTime.trim()) generalErrors.readTime = 'Read time is required.';
// //     if (!formData.metaTitle.trim()) generalErrors.metaTitle = 'Meta title is required.';
// //     if (!formData.metaDescription.trim()) generalErrors.metaDescription = 'Meta description is required.';
// //     if (!formData.imageAlt.trim()) generalErrors.imageAlt = 'ImageAlt is required.';
// //     if (!formData.keywords.trim()) generalErrors.keywords = 'Keywords are required.';
// //     if (!formData.tags.trim()) generalErrors.tags = 'Tags are required.';
    
// //     // Featured Image validation
// //     if (!editingArticle && !mainImgFile) {
// //       generalErrors.mainImage = 'Featured image is required.';
// //     }
...
// //       const selectedCategory = categories.find(cat => cat._id === formData.category);
// //       if (selectedCategory) {
// //         generalErrors.author = `Author is required for ${selectedCategory.name} category. Please ensure an author exists for this category.`;
// //       }
// //     }
    
// //     // Content validation
// //     if (!hasValidParagraph && contentBlocks.length > 0) {
// //       generalErrors.content = 'At least one paragraph block with text is required.';
// //     } else if (contentBlocks.length === 0) {
// //       generalErrors.content = 'Please add at least one content block (paragraph required).';
// //     }
    
// //     setFormErrors(generalErrors);
    
// //     // If there are any errors, stop submission
// //     if (Object.keys(generalErrors).length > 0 || Object.keys(blockErrors).length > 0) {
...
// //     setSaving(true);
// //     try {
// //       // Strip frontend-only imageFile / imagePreview before sending JSON
...
// //       // Append ONLY blocks that have a new file (src === '__NEW_UPLOAD__')
// //       contentBlocks.forEach((b) => {
// //         if (b.type === 'image' && b.imageFile) fd.append('contentImages', b.imageFile);
// //       });
...
// //     try {
// //       await articlesAdminAPI.delete(confirm.id);
// //       await fetchArticles(selectedCat);
// //     } catch (err) {
...
// //       });
// //       // Existing image blocks keep their real src; imagePreview shows the saved image
// //       setContentBlocks(
// //         (article.content || []).map((b) =>
// //           b.type === 'image'
// //             ? { ...b, imageFile: null, imagePreview: b.src ? `http://localhost:5000${b.src}` : '' }
// //             : { ...b }
// //         )
// //       );
// //       setMainImgPreview(article.image ? `http://localhost:5000${article.image}` : '');
// //       setMainImgFile(null);
// //       // Set category name for display
// //       const selectedCategory = categories.find(cat => cat._id === (article.category?._id || article.category));
// //       if (selectedCategory) setSelectedCategoryName(selectedCategory.name);
// //     } else {
...
// //           className="flex items-center gap-2 px-4 py-2 bg-[#F5C645] text-black rounded-lg hover:bg-[#F5C645]/90 transition-all cursor-pointer font-medium">
...
// //               selectedCat === c._id ? 'bg-[#F5C645] text-black font-semibold' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'
// //             }`}>
// //             {c.name}
// //           </button>
// //         ))}
...
// //             <div key={article._id}
// //               className="bg-gradient-to-br from-gray-900 to-black border border-[#F5C645]/20 rounded-2xl p-5 hover:border-[#F5C645]/40 transition-all">
// //               <div className="flex flex-col lg:flex-row gap-5">
// //                 {article.image && (
// //                   <div className="lg:w-44 h-28 rounded-xl overflow-hidden shrink-0">
// //                     <img src={`http://localhost:5000${article.image}`} alt={article.title} className="w-full h-full object-cover" />
// //                   </div>
// //                 )}
...
// //                         </span>
// //                       </div>
// //                       <h3 className="text-white text-lg font-semibold mb-1 truncate">{article.title}</h3>
// //                       <p className="text-gray-400 text-sm line-clamp-2">{article.excerpt}</p>
// //                       <div className="flex gap-4 mt-2 text-gray-500 text-xs">
// //                         <span className="flex items-center gap-1"><FiCalendar size={11} />{new Date(article.date).toLocaleDateString()}</span>
...
// //                         <button className="p-2 text-green-400 hover:bg-green-400/10 rounded-lg cursor-pointer"><FiEye size={17} /></button>
// //                       </Link>
// //                       <button onClick={() => openModal(article)} className="p-2 text-blue-400 hover:bg-blue-400/10 rounded-lg cursor-pointer"><FiEdit2 size={17} /></button>
// //                       <button onClick={() => setConfirm({ open: true, id: article._id, title: article.title })}
// //                         className="p-2 text-red-400 hover:bg-red-400/10 rounded-lg cursor-pointer"><FiTrash2 size={17} /></button>
// //                     </div>
// //                   </div>
// //                 </div>
// //               </div>
...
// //       {/* ══════════════════════ MODAL ══════════════════════ */}
// //       {showModal && (
// //         <div className="fixed inset-0 bg-black/85 overflow-y-auto z-50 p-4">
// //           <div className="max-w-4xl mx-auto my-8">
// //             <div className="bg-[#0d0d0d] border border-[#F5C645]/20 rounded-2xl">

// //               {/* Modal header */}
// //               <div className="flex justify-between items-center px-6 py-4 border-b border-gray-800 sticky top-0 bg-[#0d0d0d]/95 backdrop-blur rounded-t-2xl z-10">
// //                 <h2 className="text-white text-xl font-semibold">{editingArticle ? 'Edit Article' : 'Add Article'}</h2>
...
// //                 {/* API error */}
// //                 {formErrors.api && (
// //                   <div className="flex items-center gap-3 bg-red-950/60 border border-red-500/40 text-red-300 px-4 py-3 rounded-xl text-sm">
// //                     <FiAlertCircle size={15} />{formErrors.api}
// //                   </div>
// //                 )}
...
// //                       <input type="text" className={inpCls('title')}
...
// //                       <select className={inpCls('category') + ' cursor-pointer'}
...
// //                       <div className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white text-sm">
...
// //                           No author assigned to {selectedCategoryName} category. Please add an author with this category.
// //                         </p>
// //                       )}
// //                     </Field>
...
// //                   <p className="text-gray-500 text-xs -mt-2">Only .webp · Under 100 KB</p>
...
// //                     {mainImgPreview && (
// //                       <div className="relative shrink-0">
// //                         <img src={mainImgPreview} alt="preview" className="w-28 h-28 rounded-xl object-cover" />
...
// //                     <p className="text-red-400 text-xs mt-1">{formErrors.mainImage}</p>
// //                   )}
...
// //                         className={`${cls} text-white px-3 py-1.5 rounded-lg text-sm font-medium cursor-pointer transition-all`}>
...
// //                         </p>
// //                         {formErrors.content && (
// //                           <p className="text-red-400 text-xs mt-2">{formErrors.content}</p>
// //                         )}
// //                       </div>
// //                     ) : (
...
// //                           onMove={moveBlock}
// //                           onImagePick={pickBlockImage}
// //                           onImageClear={clearBlockImage}
...
// //                   {/* Image error toast shown directly below content blocks */}
// //                   <ImageToast message={imageToast} onClose={() => setImageToast('')} />
// //                 </Section>

// //                 {/* ── Sticky footer ── */}
// //                 <div className="flex gap-3 sticky bottom-0 bg-[#0d0d0d]/95 backdrop-blur py-4 -mx-6 px-6 border-t border-gray-800 rounded-b-2xl">
...
//   // Search filter (client-side across already-loaded articles)
//   useEffect(() => {
...
//       setContentBlocks(
//         (article.content || []).map((b) =>
//           b.type === 'image' ? { ...b, imageFile: null, imagePreview: b.src ? `http://localhost:5000${b.src}` : '' } : { ...b }
//         )
//       );
//       setMainImgPreview(article.image ? `http://localhost:5000${article.image}` : '');
//       setMainImgFile(null);
