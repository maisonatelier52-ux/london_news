...
import {
  FiPlus, FiEdit2, FiTrash2, FiEye, FiCalendar, FiClock, FiSave,
  FiX, FiHash, FiImage, FiFileText, FiAlignLeft, FiUpload, FiTrash,
  FiAlertCircle, FiSearch, FiList, FiHelpCircle, FiRotateCcw,
} from "react-icons/fi";
import { articlesAdminAPI, categoriesAdminAPI, authorsAdminAPI } from "@/services/adminAPI";
import ConfirmDialog from "@/components/admin/ConfirmDialog";
import ArticleScheduleCorrectionPanel from "@/components/admin/ArticleScheduleCorrectionPanel";
import ArticleRevisionHistoryPanel from "@/components/admin/ArticleRevisionHistoryPanel";
import Link from "next/link";
...
function AtAGlanceEditor({ block, index, onUpdate, errors }) {
  const inp = 'w-full bg-black/50 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#F5C645] transition-colors';

  function addRow() {
    const rows = [...(block.glanceRows || []), { label: '', value: '' }];
  ...
  function removeRow(ri) {
    const rows = (block.glanceRows || []).filter((_, i) => i !== ri);
    onUpdate(index, 'glanceRows', rows);
  }
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-gray-500 text-xs mb-1">Section title</label>
          <input
            type="text"
            value={block.glanceTitle || 'At a glance'}
            onChange={e => onUpdate(index, 'glanceTitle', e.target.value)}
            className={inp}
            placeholder="At a glance"
          />
        </div>
        <div>
          <label className="block text-gray-500 text-xs mb-1">Subtitle (optional)</label>
          <input
            type="text"
            value={block.glanceSubtitle || ''}
            onChange={e => onUpdate(index, 'glanceSubtitle', e.target.value)}
            className={inp}
            placeholder="e.g. Understanding the framework of influence"
          />
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-gray-600 text-xs">{(block.glanceRows || []).length} row(s)</span>
        </div>
        <div className="space-y-2">
          {(block.glanceRows || []).map((row, ri) => (
            <div key={ri} className="flex items-center gap-2">
              <input
                ...
                placeholder="Label (e.g. Core argument)"
                className="flex-1 bg-black/50 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#F5C645] transition-colors"
              />
              <input
                type="text"
                value={row.value}
                ...
                placeholder="Value"
                className="flex-[2] bg-black/50 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#F5C645] transition-colors"
              />
              <button
                type="button"
                onClick={() => removeRow(ri)}
                className="p-1.5 text-red-400 hover:text-red-300 hover:bg-red-400/10 rounded cursor-pointer shrink-0"
              >
                <FiTrash size={13} />
              </button>
            </div>
        ...
        <button
          type="button"
          onClick={addRow}
          className="mt-2 text-xs text-[#F5C645] hover:underline cursor-pointer flex items-center gap-1"
        >
          <FiPlus size={12} /> Add row
        </button>
        {errors?.[index]?.glanceRows && (
          <p className="text-red-400 text-xs mt-1">{errors[index].glanceRows}</p>
        )}
      </div>
    </div>
  );
...
function FaqEditor({ block, index, onUpdate, errors }) {
  const inp = 'w-full bg-black/50 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#F5C645] transition-colors';

  function addItem() {
    const items = [...(block.faqItems || []), { question: '', answer: '' }];
  ...
  function removeItem(ii) {
    const items = (block.faqItems || []).filter((_, i) => i !== ii);
    onUpdate(index, 'faqItems', items);
  }
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <input
          type="text"
          value={block.faqTitle || 'Frequently asked questions'}
          onChange={e => onUpdate(index, 'faqTitle', e.target.value)}
          className={inp}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-gray-500 text-xs">Questions</label>
          <span className="text-gray-600 text-xs">{(block.faqItems || []).length} item(s)</span>
        </div>
        <div className="space-y-3">
          {(block.faqItems || []).map((item, ii) => (
            <div key={ii} className="border border-gray-800 rounded-lg p-3 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <span className="text-gray-600 text-xs mt-1">Q{ii + 1}</span>
                <button
                  type="button"
        ...
        {errors?.[index]?.faqItems && (
          <p className="text-red-400 text-xs mt-1">{errors[index].faqItems}</p>
        )}
      </div>
    </div>
  );
...
function ContentBlock({ block, index, total, onUpdate, onRemove, onMove, onImagePick, onImageClear, errors }) {
  const tagColor = {
    ...
    pullquote:    'bg-green-900/40 text-green-400 border-green-700/40',
    image:        'bg-orange-900/40 text-orange-400 border-orange-700/40',
    at_a_glance:  'bg-yellow-900/40 text-yellow-400 border-yellow-700/40',
    faq:          'bg-pink-900/40 text-pink-400 border-pink-700/40',
  };
  const labelMap = {
    paragraph: 'paragraph', subheading: 'subheading', pullquote: 'pullquote',
  ...
  return (
    <div className="rounded-xl border border-gray-800 overflow-hidden bg-[#111]">
      ...
      <div className="p-3 sm:p-4 space-y-2">
        {block.type === 'paragraph' && (
          <>
            <textarea value={block.text} onChange={e => handleUpdate('text', e.target.value)}
              placeholder="Enter paragraph text…" rows={3} className={`${inp} ${blockError?.text ? errorInp : ''}`} />
            {blockError?.text && <p className="text-red-400 text-xs mt-1">{blockError.text}</p>}
          </>
        )}
        ...
        {block.type === 'subheading' && (
          <>
            <input type="text" value={block.text} onChange={e => handleUpdate('text', e.target.value)}
              placeholder="Enter subheading…" className={`${inp} ${blockError?.text ? errorInp : ''}`} />
            {blockError?.text && <p className="text-red-400 text-xs mt-1">{blockError.text}</p>}
          </>
        )}
        ...
        {block.type === 'pullquote' && (
          <>
            <textarea value={block.text} onChange={e => handleUpdate('text', e.target.value)}
              placeholder="Quote text…" rows={2} className={`${inp} ${blockError?.text ? errorInp : ''}`} />
            {blockError?.text && <p className="text-red-400 text-xs mt-1">{blockError.text}</p>}
            <input type="text" value={block.attribution} onChange={e => handleUpdate('attribution', e.target.value)}
              placeholder="Attribution (optional)" className={inp} />
          </>
        )}
        ...
        {block.type === 'image' && (
          <div className="space-y-2">
            ...
            {blockError?.src && <p className="text-red-400 text-xs">{blockError.src}</p>}
            <input type="text" value={block.alt} onChange={e => handleUpdate('alt', e.target.value)} placeholder="Alt text (SEO)" className={inp} />
            <input type="text" value={block.caption} onChange={e => handleUpdate('caption', e.target.value)} placeholder="Caption (optional)" className={inp} />
          </div>
        )}

        {block.type === 'at_a_glance' && (
          <AtAGlanceEditor block={block} index={index} onUpdate={onUpdate} errors={errors} />
        )}

        {block.type === 'faq' && (
          <FaqEditor block={block} index={index} onUpdate={onUpdate} errors={errors} />
        )}
      </div>
    </div>
  );
...
const EMPTY_FORM = {
  title: '', slug: '', metaTitle: '', metaDescription: '', excerpt: '',
  category: '', topic: '', newsType: 'news', type: 'normal',
  date: new Date().toISOString().split('T')[0],
  readTime: '', isPublished: true, imageAlt: '', keywords: '', tags: '', author: '',
};
...
export default function ArticlesPage() {
  ...
  function handleCategoryChange(categoryId) {
    setFormData(prev => ({ ...prev, category: categoryId, topic: '' }));
    setFormErrors(prev => ({ ...prev, category: '' }));
    const autoAuthorId = getAuthorByCategory(categoryId);
    setFormData(prev => ({ ...prev, category: categoryId, author: autoAuthorId || '' }));
    setTopics([]);
    if (categoryId) {
      categoriesAdminAPI.getTopics(categoryId)
  ...
  function addBlock(type) {
    const defaults = {
      ...
      pullquote:   { type, text: '', attribution: '' },
      image:       { type, src: '', imageFile: null, imagePreview: '', alt: '', caption: '' },
      at_a_glance: { type, glanceTitle: 'At a glance', glanceSubtitle: '', glanceRows: [{ label: '', value: '' }] },
      faq:         { type, faqTitle: 'Frequently asked questions', faqItems: [{ question: '', answer: '' }] },
    };
    setContentBlocks(p => [...p, defaults[type] || { type }]);
    if (formErrors.content) setFormErrors(prev => ({ ...prev, content: '' }));
  }
  ...
  function validateContentBlocks() {
    ...
    contentBlocks.forEach((block, idx) => {
      ...
      if (block.type === 'at_a_glance') {
        if (!block.glanceRows?.length || !block.glanceRows.some(r => r.label?.trim() && r.value?.trim()))
          blockErr.glanceRows = 'Add at least one row with label and value.';
      }
      if (block.type === 'faq') {
        if (!block.faqItems?.length || !block.faqItems.some(f => f.question?.trim() && f.answer?.trim()))
          blockErr.faqItems = 'Add at least one question and answer.';
      }
      if (Object.keys(blockErr).length > 0) errors[idx] = blockErr;
    });
    setContentErrors(errors);
  ...
  async function handleSubmit(e) {
    ...
    if (!formData.title.trim())           ge.title           = 'Title is required.';
    if (!formData.slug.trim())            ge.slug            = 'Slug is required.';
    else if (!isValidSlug(formData.slug)) ge.slug            = 'Slug must contain only lowercase letters and hyphens.';
    if (!formData.category)               ge.category        = 'Category is required.';
    if (!formData.excerpt.trim())         ge.excerpt         = 'Excerpt is required.';
    else if (countWords(formData.excerpt) < 20) ge.excerpt   = `Excerpt must contain at least 20 words. Currently ${countWords(formData.excerpt)} word(s).`;
    if (!formData.readTime.trim())        ge.readTime        = 'Read time is required.';
    ...
    try {
      ...
      if (!articleData.author) delete articleData.author;
      if (!articleData.topic) delete articleData.topic;
      const fd = new FormData();
      fd.append('data', JSON.stringify(articleData));
      if (mainImgFile) fd.append('mainImage', mainImgFile);
      contentBlocks.forEach(b => { if (b.type === 'image' && b.imageFile) fd.append('contentImages', b.imageFile); });
  ...
  function openModal(article = null) {
    if (article) {
      ...
      const deskId = article.category?._id || article.category || '';
      setTopics([]);
      if (deskId) {
        categoriesAdminAPI.getTopics(deskId).then(r => setTopics(r.data || [])).catch(() => setTopics([]));
      }
      setContentBlocks(
        (article.content || []).map(b =>
          b.type === 'image'
              ...
              <form onSubmit={handleSubmit} className="px-4 sm:px-6 pb-6 pt-4 space-y-6 sm:space-y-8">
                ...
                <Section icon={<FiFileText size={16} />} title="Basic Information">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Field label="Slug" required error={formErrors.slug}>
                      <input type="text" {...inp('slug')} className={inpCls('slug')} placeholder="my-article-title" />
                      <p className="text-gray-500 text-xs mt-1">Only lowercase letters and hyphens</p>
                    </Field>
                    <Field label="Category" required error={formErrors.category}>
                      <select className={inpCls('category') + ' cursor-pointer'} value={formData.category}
                        onChange={e => handleCategoryChange(e.target.value)}>
                        <option value="">Select Category</option>
                        {categories.filter(c => c.type !== 'topic').map(c => <option key={c._id} value={c._id}>{c.name}</option>)}
                      </select>
                    </Field>
                    {topics.length > 0 && (
                      <Field label="Topic (optional)">
                        <select className={inpCls('topic') + ' cursor-pointer'} value={formData.topic}
                          onChange={e => setFormData(prev => ({ ...prev, topic: e.target.value }))}>
                          <option value="">No specific topic</option>
                          {topics.map(t => <option key={t._id} value={t._id}>{t.name}</option>)}
                        </select>
                        <p className="text-gray-500 text-xs mt-1">Tags this article under a child topic (e.g. Markets, Westminster) so it also appears on /topics/[slug].</p>
                      </Field>
                    )}
                    <Field label="Author" required error={formErrors.author}>
                      <div className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white text-sm min-h-[38px] flex items-center">
                ...
                {/* Scheduling & Corrections — only once the article has been saved at
                    least once (has an _id + previewToken from the backend) */}
                {editingArticle?._id && (
                  <Section icon={<FiClock size={16} />} title="Scheduling & Corrections">
                    <ArticleScheduleCorrectionPanel
                      article={editingArticle}
                      onUpdated={(updated) => setEditingArticle((prev) => ({ ...prev, ...updated }))}
                    />
                  </Section>
                )}

                {/* Revision history — one snapshot is saved automatically on every
                    update (see PUT /articles/:id), so this only appears once the
                    article has been saved at least once. Restoring re-hydrates the
                    whole form (title/content/image/etc.), not just editingArticle,
                    via the same openModal() logic used when the editor first opens. */}
                {editingArticle?._id && (
                  <Section icon={<FiRotateCcw size={16} />} title="Revision History">
                    <ArticleRevisionHistoryPanel
                      article={editingArticle}
                      onUpdated={(updated) => openModal(updated)}
                    />
                  </Section>
                )}
                ...
                <Section icon={<FiAlignLeft size={16} />} title="Content">
                  ...
                  {/* <div className="flex gap-2 flex-wrap">
                    {[
                      { type: 'paragraph',   cls: 'bg-blue-700 hover:bg-blue-600',   label: '+ Paragraph' },
                      { type: 'subheading',  cls: 'bg-purple-700 hover:bg-purple-600', label: '+ Subheading' },
                      { type: 'pullquote',   cls: 'bg-green-700 hover:bg-green-600',  label: '+ Pullquote' },
                      { type: 'image',       cls: 'bg-orange-700 hover:bg-orange-600', label: '+ Image' },
                      { type: 'at_a_glance', cls: 'bg-yellow-700 hover:bg-yellow-600', label: '+ At a Glance' },
                      { type: 'faq',         cls: 'bg-pink-700 hover:bg-pink-600',    label: '+ FAQ' },
                    ].map(({ type, cls, label }) => (
                      <button key={type} type="button" onClick={() => addBlock(type)}
                        className={`${cls} text-white px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium cursor-pointer transition-all flex items-center gap-1`}>
                        {type === 'at_a_glance' && <FiList size={13} />}
                        {type === 'faq' && <FiHelpCircle size={13} />}
                        {label}
                      </button>
                    ))}
                  </div> */}
